...
//      emailRequest.setBody("Welcome to mate flix. kindly verify your account. Your verification code is    " + savedUser.getToken() + "    or click on this link    "  + verificationLink);
        String htmlBody = readHtmlFile("Header.html");
        htmlBody = htmlBody.replace("{{email}}", savedUser.getEmail());
        htmlBody = htmlBody.replace("{{token}}", verificationLink);
//    emailRequest.setSubject("MATE FLIX EMAIL VERIFICATION");
    ...
    private String readHtmlFile(String fileName) {
        try {
            ...
            byte[] fileContent = FileCopyUtils.copyToByteArray(classPathResource.getInputStream());
            return new String(fileContent, StandardCharsets.UTF_8);
        } catch (IOException e) {
            throw new RuntimeException("Failed to read HTML file: " + fileName, e);
        }
    }
